...
### [2026-04-19] Data integrity: бэкфилл, агрегация TF, уведомления, UX-фиксы
...
- Функция `_aggregate_1m(candles_1m, tf, tf_min)`: группирует по выровненным временным окнам, пропускает неполные свечи
- `_save_with_aggregates(candles_1m, repo)`: сохраняет 1m + 5m + 15m + 1h + 4h + 1d за один вызов
- `run_backfill` (авто-старт): загружает 2000 последних 1m-свечей если в БД меньше
- `run_manual_backfill`: пагинация назад от `now_ms` по всему запрошенному периоду, агрегирует и сохраняет

**Очистка и пересчёт БД:**
- Добавлен `CandlesRepository.delete_timeframe(symbol, tf)` для очистки одного TF
- Все 5 пар (BTC, ETH, SOL, BNB, XRP) очищены от некорректных агрегированных TF и пересчитаны из 1m-свечей
- BNB/USDT: 10092 1m → 2017 5m → 671 15m → 167 1h → 41 4h → 6 1d (корректные пропорции)
...
- Раньше: `useEffect(() => { onRequestStats() }, [])` — запрос отправлялся при монтировании, но если WS ещё не открыт (обновление страницы, `activeTab='data'` сохранён в persist) — команда молча отбрасывалась
- Теперь: `useEffect(() => { if (connected) onRequestStats() }, [connected])` — запрашивает как только соединение установлено, и повторно при реконнекте

**Решения:**
- Агрегация из 1m гарантирует математическую согласованность: 5m-свеча = ровно 5 минутных, границы всегда выровнены, нет «артефактных» данных из API
- `startBackfill` в `useWebSocket` (а не в `DataView`) — единственное место где есть `notifMapRef`, создание и трекинг уведомления в одном месте без prop-drilling
- Реакция на `connected` вместо `[]` в useEffect — единственный надёжный способ дождаться открытого WS-соединения перед отправкой команды

**Отложено:**
- Отображение прогресса бэкфилла в виде timeline на вкладке Данные (после завершения — обновить DataView без перезапроса)

Тесты:
  Unit:        —
  Integration: —
  Smoke:       ✅ бэкфилл для 5 пар, проверка пропорций в БД
  Покрытие:    н/д

Коммит: `—`
Следующий шаг: Продолжить загрузку исторических данных, затем AI Advisor / ML Dataset (Phase 1-G)
