...
**Исправлен критический баг парсинга BingX API (`data/bingx_rest.py`):**
- BingX v3 `/openApi/swap/v3/quote/klines` возвращает свечи как JSON-объекты `{"time": ms, "open": "...", ...}`, а не массивы `[time, open, ...]`
- Код использовал `row[0]`, `row[1]` — каждая свеча падала с `KeyError` и молча пропускалась
- Результат: 521 реальная 1m-свеча в БД (из live WS), а не исторические данные
- Исправлено на `row["time"]`, `row["open"]`, `row["high"]`, `row["low"]`, `row["close"]`, `row["volume"]`

**Стратегия бэкфилла: только 1m → агрегация (`data/backfill.py`):**
- Обнаружена аномалия в БД: 5m-свечей больше чем 1m-свечей (каждый TF загружался независимо с разными временными окнами)
- Принято решение: загружать только `1m` через REST API, все остальные TF агрегировать локально
- `_aggregate_1m(candles_1m, tf, tf_min)`: группирует по выровненным временным окнам, пропускает неполные свечи
- `_save_with_aggregates(candles_1m, repo)`: сохраняет 1m + все AGG_TFS за один вызов
- `run_backfill` (авто-старт): загружает 2000 последних 1m-свечей если в БД меньше
- `run_manual_backfill`: пагинация назад по всему запрошенному периоду, агрегирует и сохраняет

**Полный список агрегируемых TF (`data/backfill.py`):**
- Исходно `AGG_TFS` содержал только 5m/15m/1h/4h/1d — 3m/30m/2h отсутствовали
- Добавлены `3m (3), 30m (30), 2h (120)` → теперь `AGG_TFS`: 3m/5m/15m/30m/1h/2h/4h/1d
- Исправлены накопленные стале-данные `repair_integrity()` (авторемонт при старте)

**Авторемонт целостности TF при старте (`data/backfill.py → repair_integrity()`):**
- Проверяет пропорции: `|факт - ожид| / ожид > 10%` для каждого TF
- При нарушении: удаляет все агрегированные TF и пересчитывает из 1m-свечей
- Запускается автоматически при каждом `main.py` перед backfill-ом
- Результат: накопленные нарушения для всех 5 пар исправлены за один старт

**Обновление последних 48 часов из REST API (`data/backfill.py → refresh_recent()`):**
- Новая функция: перезаписывает последние 48 часов 1m-свечей из REST при каждом старте
- 2 запроса по 1440 свечей = полное покрытие 48h (API max = 1440 за запрос)
- Устраняет WS-артефакты: WebSocket детектирует закрытие свечи по смене `open_time` следующего тика — биржа может ещё не завершить обработку последних трейдов
- Цепочка запуска: `repair_integrity → refresh_recent → run_backfill`

**Очистка и пересчёт БД:**
- Добавлен `CandlesRepository.delete_timeframe(symbol, tf)` для очистки одного TF
- Все 5 пар очищены от некорректных агрегированных TF и пересчитаны из 1m-свечей
- BTC/USDT: 43 225 1m → 14 407 3m → 8 644 5m → 2 881 15m → 1 440 30m → 720 1h → 360 2h → 179 4h → 29 1d

**SQLite WAL-режим (`storage/database.py`):**
- `PRAGMA journal_mode=WAL` — параллельные читатели при одном писателе (без блокировок)
- `PRAGMA busy_timeout=30000` — 30с ожидание вместо мгновенного `database is locked`
- `PRAGMA synchronous=NORMAL` — баланс надёжности и скорости
- `connect_args={"timeout": 30}` — таймаут на уровне драйвера aiosqlite

**Сжатие логов (`core/logger.py`):**
- `LOG_LEVEL=DEBUG → INFO` в `.env` — прекращён поток отладочных сообщений
- Файловые логи всегда `INFO` независимо от `LOG_LEVEL`
- `retention=7` файлов вместо `"30 days"` — предотвращает накопление ГБ логов
- `compression="gz"` — ротируемые файлы сжимаются автоматически
- Убраны `log.debug()` из `EventBus.subscribe/publish` — генерировали 25+ строк/сек от candle-событий
- Размер проекта: 913 МБ → ~50 МБ (после очистки старых логов)

**BackfillModal: мультиселект пар (`ui/react-app/src/components/DataView.tsx`):**
- Выбор нескольких пар для загрузки (toggle-кнопки, синяя рамка = выбрана)
- Кнопка «Все пары / Снять все» для быстрого выбора
- По умолчанию выбраны все 5 пар
- Кнопка старта адаптируется: «Загрузить 5 пар» / «Начать загрузку»
- Оценка времени масштабируется на кол-во выбранных пар

**Мгновенные уведомления о загрузке (`ui/react-app/src/hooks/useWebSocket.ts`):**
- Новая функция `startBackfill(symbol, period)` экспортируется из `useWebSocket`
- При клике: сразу создаёт уведомление (`addNotification`) и прописывает `taskId → notifId` в `notifMapRef`
- Панель «Активные загрузки» в модале появляется мгновенно, не ждёт ответа бэкенда
- WS-события `backfill.progress` обновляют существующее уведомление через `updateNotification` (без дублирования)

**Автозагрузка статистики БД (`ui/react-app/src/components/DataView.tsx`):**
- Раньше: `useEffect([], [])` — команда отправлялась при монтировании до открытия WS → молча отбрасывалась
- Теперь: `useEffect(() => { if (connected) onRequestStats() }, [connected])` — запрашивает как только WS установлен, повторяет при реконнекте

**Кросс-валидация данных vs BingX API (`scripts/validate_candles.py`):**
- Новый скрипт: 3 случайных 50-свечных окна на каждую пару из БД vs REST API BingX
- Допуск цены `0.1%`, объёма `0.1%` — реалистичен для WS vs REST settlement pipeline
- Результат после всех исправлений: 5/5 пар ✓ OK, 750 проверок, 0 несовпадений
- Выявил природу WS-артефактов: BingX дообрабатывает свечи в фоне, суб-0.1% — штатное поведение
- Windows-совместимость: `aiohttp.TCPConnector(resolver=aiohttp.ThreadedResolver())` вместо aiodns

**Решения:**
- Агрегация из 1m гарантирует математическую согласованность всех TF — нет «артефактных» данных из API
- `repair_integrity` при каждом старте — защита от накопления дрейфа пропорций TF при WS-прерываниях
- `refresh_recent` = 48h (2 × 1440) — покрывает всё WS-окно с запасом; один старт исправляет всё
- `PRICE_TOL = 0.001` в валидации — BingX меняет значения закрытых свечей в пределах 0.1% до полного settlement
- WAL + busy_timeout: SQLite без блокировок при параллельных чтениях аналитики и записи WS-данных
- `startBackfill` в `useWebSocket` (а не `DataView`) — `notifMapRef` только здесь, без prop-drilling

**Отложено:**
- Отображение прогресса бэкфилла в виде timeline (после завершения — обновить DataView без перезапроса)

Тесты:
  Unit:        —
  Integration: —
  Smoke:       ✅ 5/5 пар, 750 проверок vs BingX API, 0 несовпадений
  Покрытие:    н/д

Коммиты: `050c2dd` `744925c` `8512365` `76fe105` `bd5f6c6`
Следующий шаг: AI Advisor / ML Dataset (Phase 1-G)
